		...
		switch (action.type) {
			...
			case "bodyRules":
				if (!action.value.trim()) {
					...
					draft.body.message = "You must provide content to the body field"
				}
				return
			case "notFound":
				draft.notFound = true
			...
			try {
				...
				if (response.data) {
					...
					if (appState.user.username != response.data.author.username) {
						appDispatch({ type: "flashMessage", value: "You don't have access to edit this post" })
						// redirect
						props.history.push("/")
					}
				} else {
		...
		<Page title="Edit New Post">
			...
			<form className="mt-3" onSubmit={submitHandler}>
				...
				<button className="btn btn-primary" disabled={state.isSaving}>
					{state.isSaving ? "Saving..." : "Save Updated Post"}
				</button>
			</form>
		</Page>
	)
